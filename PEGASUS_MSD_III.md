...
- **Cross-cutting (engineering-debt remediation — §V.1/§V.7 contracts; sequenced opportunistically, not phase-gated):** `NAT-01` national acquisition + combine + national compile scope (all 27 UFs → national municipality panel) — **landed**; `STORE-01` storage lifecycle contracts V.7(1,2,4) (single-serialization DATASUS v4 bridge, slim SIDRA, non-inlining manifests, ZSTD, GC) — **landed**; `STORE-02` V.7(3) lazy `scan_parquet` views over Hive-partitioned canonical/facts (retire the `combined`/`canonical`/`national` re-materialization) — **open**; `POP-02` the §V.1 tensor compute contract (above) — **M1/M3/M4/M5/M7 landed** (numpy-array problem storage + exact locality-blocked solve + vectorized emission + **numpy-array loss/solver returns**, the last being the true hotspot — `tuple(float)` gradient round-trips were ~99% of a loss eval); **GPU port now justified** (the dense math dominates post-M7; ~15× measured) + solve parallelization + M6 input-construction memory remain. `FAL-POP` national+full-history scope-invariant population-tensor asset (§VI.1/§II.4) — **data pipeline landed** (the tensor now ingests all three census anchors — 2000 via SIDRA 2093 CTR-disaggregated against the 2010 shape, 2010/2022 via 9606 — and builds over the full history regardless of the query window, scope-invariant acquire→compile→build; `interpolate_census_composition` is correctly the §II.4 Layer-1 prior-mean). **Remaining sub-items:** `FAL-POP-SV` **single-vintage re-anchoring** — intercensal closure totals from census-anchored cohort projection, not the raw 6579 series (the §II.4 anti-discontinuity contract; the 2021→2022 ~5% jump is 6579-projection-vs-census vintage mixing and is resolved *only* by this change, not by full-range alone); `FAL-POP-PROJ` **native forward/backward extrapolation** beyond official anchors via cohort-component projection with anchor-distance-growing, `fragile`-typed uncertainty (§II.4); `FAL-POP-VER` **build-once versioned asset + slice-on-query** (§VI.2/§VI.3 — the asset still rebuilds per-run, now at full scope; needs the version store + `resolve_asset` asserting `build_scope=='national_full_history'` + compile-slices-not-rebuilds); `FAL-POP-VAL` **national demographic-sanity validation** of the full-history reconstruction (totals vs each census, closure, age pyramid, sex/race composition, single-vintage trend continuity) — the certifying proof.
