...
#### `FAL-POP` — Population tensor as a scope-invariant foundational asset — ⬜ OPEN (the deeper issue)
- **MSD-III:** §VI.1 ("Building a scope-invariant asset at reduced scope is a **correctness bug**"), §II.4 ("all three censuses 2000/2010/2022 MUST be ingested regardless of a query's time window; a query *slices*, never re-scopes, the tensor"). **Files:** `workflows/compile.py` (builds the tensor PER-RUN at the intent window), `sidra/population_cube/build.py` → TDD Phase-6 `FAL-01`/`assets/`.
- **The violation (confirmed by the audit + validation):** the run **rebuilds the tensor per query at the intent's year window** — the validated national tensor covers **only 2021–2022** because that was the intent window. This (a) violates §VI.1 scope-invariance, (b) violates §II.4's all-three-censuses rule, and (c) leaves the 2000–2025 pancreatic-cancer study with **no denominator for 24 of 26 years**. `interpolate_census_composition` is *correctly* prescribed (§II.4 Layer 1 warm-start — not "naive interpolation"); the bug is the *scope* it is built at.
- **Target:** build the population tensor ONCE as a versioned national + full-history (2000–2025, all three censuses) foundational asset (Phase-6 `assets/` + `FAL-01`); queries slice it. **User decisions (locked):** cohort-project between censuses (§II.4 Layer-2 process solver) · single-vintage anchoring (census-anchored, not the 6579 series that causes the 2021→2022 jump) · build `FAL-POP` next.
- **Build sequence (concrete):**
  - **#1 — census-scope invariance** — ✅ LANDED (`f715dd6`): `_all_census_periods` ingests every census the strata table declares regardless of the query window; `test_population_scope_invariance.py`.
  - **#2 — register + acquire table 2093 (the 2000-census strata)** — 🟢 registered (`pipeline.py` `SIDRA_CENSUS_2000_STRATA_*` constants). *Confirmed live structure:* 2093 has periods {2000, 2010}; race (clsf 86, 7 cats), sex (clsf 2), **age = 19 GROUPS (clsf 58)**, urban/rural situation (clsf 1, pin Total). **Remaining:** (a) register 2093's 19 age-group categories in `config/registries/…/demographic_axis_maps.yaml` so `map_category('age_group', 'SIDRA', …)` resolves them; (b) `_acquire_sidra_census_2000_strata` fetching 2093@2000 (race×sex×age-group, situation=Total); (c) wire into `_acquire_national`.
  - **#3 — CTR age disaggregation** (§II.4 "clean partition selected, roll-ups derived, never summed"): disaggregate 2093's 19 age *groups* → the tensor's single-year axis via the `reconstruction/ctr.py` CTR (proportional-to-2010-single-year-shape within each 2000 bracket, preserving bracket totals). Yields a 2000 single-year age×sex×race anchor consistent with 9606's 2010/2022.
  - **#4 — full-history cohort-projected build**: build the tensor over **2000–2025** (not the query window) with all three census anchors; the §II.4 Layer-2 process solver cohort-ages between them; **single-vintage** closure totals (census-anchored + projected, not 6579). Compose with `POP-02` (numpy/blocked/parallel/GPU) — this is the run that most needs those perf items.
  - **#5 — version + store** (Phase-6 `assets/{tiers,version,store}.py`): immutable `population_tensor@vYYYY.N ← {census: 2000/2010/2022, code:<hash>, certification}`; a query pins the version it consumed.
  - **#6 — slice-on-query**: `compile.py` resolves the asset (`resolve_asset`, `assert build_scope=='national_full_history'`) and **slices** to the intent window instead of rebuilding.
