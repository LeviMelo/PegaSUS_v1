...
- **Build sequence (concrete):**
  ...
  - **`FAL-POP-AMC` — temporal municipality-boundary harmonization** — ⬜ OPEN (§II.4, **support-harmonization, distinct from RECON**): a municipality created between censuses (AL: 2703759 Jequiá da Praia, absent from the 2000 census, all-non-numeric in 2093) has no census-year enumeration; the build interpolates its pre-existence 2000 population (14,682) → double-counts its parent → +0.5% AL 2000 emitted-total overshoot (the anchor itself is exact; the overshoot is the solver/warm-start filling the un-enumerated locality). **Fix needs a temporal boundary crosswalk (IBGE municipality creation/split history — NOT onboarded; `geo.amc.contract_to_amc` machinery exists but no crosswalk dataset).** Two approaches: **carve** (subtract child's estimated pre-existence pop from parent — keeps municipal resolution + the total) or **contract to AMC** (aggregate to time-invariant units — coarser resolution). Honest interim: type pre-creation cells `fragile`/boundary-derived and exclude them from the census-total invariant, never silently inflate. **Decision needed:** carve vs AMC-contract vs interim (resolution/effort tradeoff).
