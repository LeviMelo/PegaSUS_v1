...
#### `POP-02` — Population-tensor compute contract (§V.1) — ✅ LANDED (M1+M3+M4+G3; float32/GPU deferred)
...
- **Green — landed:** **(M1)** `PopulationTensorProblem` stores float64 numpy arrays (None→NaN sentinel); `loss.py`/solvers read them directly (removed the per-iteration tuple→array rebuild); `build.py` constructs with arrays — commit `POP-02 M1`. **(M3)** `solve_population_tensor_blocked` solves in ~2M-cell locality-blocks (exact — the objective is locality-separable when `migration_totals is None`; each block takes the fast dense path); peak memory `O(block)` — commit `POP-02 M3`. **(M4)** bounded-BFS migration — commit `6bb12a7`. **(G3)** deleted the orphaned `she/reconstruction/torch_kernels.py` and removed `population_tensor_kernels` from `compute.yaml` (§V.1c: no aspirational flag over unwired code). Tests: `test_population_tensor_memory.py` (numpy storage + blocked==whole exactness). **(M5)** vectorized tensor emission — the output loop built one dict per cell (~13 GB for 16.7M national cells); replaced with `np.repeat`/`tile` label columns + the flat solution arrays (~1 GB) — commit `POP-02 M5`. **Verified:** the 2-year national demographic tensor (all 5,570 munis, single-year age × sex × race) builds end-to-end `OK` at ~14.5 GB peak (was OOM-adjacent). **Deferred:** **(M6)** *input-construction memory* — `interpolate_census_composition` + the strata→anchors/closure build materialize several `O(n_cells)` Python lists + nested dicts (~14.5 GB peak for the 2-yr window; the remaining bottleneck for the full 2000–2024 window). Fix: extend the M3 locality-block boundary to the *whole* build-solve-emit pipeline (build each block's inputs, solve, stream its tensor rows) so input prep is also `O(block)` — the input path is locality-separable exactly as the solve is. **(M2)** float32 — the objective is ~1e14 with values ~1e7, so float32 risks precision; MSD-III "float64 on reductions" governs. **(G1)** GPU port — the CPU-blocked solve is fast and fits; marginal. **Done (this scope):** 2-yr national builds within the envelope on CPU; numbers unchanged (313 tests green). Full-window national needs M6.
