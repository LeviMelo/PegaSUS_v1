...
- **Build sequence (concrete):**
  ...
  - **`FAL-POP-PROJ` — native projection beyond official anchors** — ⬜ OPEN (§II.4): continue the cohort-component dynamics past the last EstimaPOP/census anchor (forward to the current year; backward before the earliest) — aging the latest reconstructed state with modelled births/deaths/migration. Uncertainty **grows monotonically with distance from the nearest anchor**, propagated + typed `state ≤ fragile`; the projection horizon is recorded in the asset manifest (§VI.2). *Care item:* this is standard cohort-component demography but its reliability envelope must be honest — no fabricated structure, only forward integration of the process.
  - **`FAL-POP-VER` — version + store + slice-on-query** — ⬜ OPEN (Phase-6 `assets/{tiers,version,store}.py`): immutable `population_tensor@vYYYY.N ← {census, estimapop, anchored_range, projection_horizon, vintage, code:<hash>, certification}`; `compile.py` resolves the asset (`resolve_asset`, `assert build_scope=='national_full_history'`) and **slices** to the intent window instead of rebuilding; a query pins the version it consumed.
  - **`FAL-POP-VAL` — national demographic-sanity validation** — ⬜ OPEN (the certifying proof): rebuild the full-history national tensor and audit totals vs *each* census (2000/2010/2022), closure residual, age-pyramid plausibility, sex/race composition per census year, and **single-vintage trend continuity** across 2021→2022. Nothing downstream trusts the tensor until this passes.
