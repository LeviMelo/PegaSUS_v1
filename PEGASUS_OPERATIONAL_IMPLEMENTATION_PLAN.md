...
- **Target:** build the population tensor ONCE as a versioned national + full-history (2000–2025, all three censuses) foundational asset (Phase-6 `assets/` + `FAL-01`); queries slice it. **Open decisions for the user:** (i) how to cover non-census years 2000–2020 / 2023–2025 (back/forward projection at the same census-anchored rigor?); (ii) the 2021(6579) vs 2022(9606) ~10 M / 4.8% vintage discontinuity — single-vintage anchoring for trends or explicit flagging?; (iii) joint age×sex×race denominator — the tensor emits only 1-D marginals, so cross-classified rates need a documented IPF/independence reconstruction.
