...
| §3.10 | **Named Core Seed Registry** + `mandatory_fields` enforcement | `core_seed_registry.yaml`, `efg/core_seed_registry.py` | maternal-child V_* + all-source 15 descriptive names resolve |
...
| §3.11 | **ICD cause-specific mortality** (σ_C chapter/block restriction) | `efg/diagnostic_strata.py` + executor `_count_tensor` | partition exact; I00–I99 leads (correct AL profile) |
...
| MSD | Module | Status |
|---|---|---|
| §2.8.3+ | `she/population/{solvers,sparse_admm,projected_gradient,block_coordinate,loss}.py` | Real projected-gradient + sparse-ADMM backends, solver selection, dense-national abort. `workflows/population.py` exists but compile never calls it; `solve_population_tensor_from_sidra_anchor` currently builds a **degenerate 1×1×1×1×1 problem** — the multi-strata (age×sex×race) orchestration with §2.8.4–2.8.10 loss terms is unbuilt. |
...
- **§2.8 demographic-tensor solver orchestration** for `independent_population_tensor` /
  `sim_informed_population_tensor` modes — build a real multi-strata `PopulationTensorProblem`
  (munis×years×age×sex×race) from disaggregated SIDRA + SIM death priors + closure totals,
  run `solve_population_tensor_problem`. (Solver backends exist; orchestration does not.)
