...
def main() -> None:
    ...
    if "sidra_stdfm_candidate" in by_id:
        row = by_id["sidra_stdfm_candidate"]
        meta = _field_metadata(row)
        if row.get("state") not in {"fragile", "verified"} or str(row.get("dashboard_safe")) != "False":
            errors.append("ST-DFM fitted candidate has invalid state/dashboard safety")
        if not meta or meta.get("stdfm_output", {}).get("status") not in {"uncertified", "verified"}:
            errors.append("ST-DFM fitted metadata missing")
    ...
    if any("blocked_solver_pending" in row.get("reason", "") for row in failed):
        errors.append("FailedBranches still claims ST-DFM solver pending")
    ...
    if sidra_context.get("stdfm_output", {}).get("status") not in {"uncertified", "verified"}:
        errors.append("ReproducibilityManifest missing ST-DFM fitted metadata")
    for table in [
        ...
        "stdfm_latent_factors.parquet",
        "stdfm_loadings.parquet",
        "stdfm_reconstructed_fields.parquet",
        "stdfm_uncertainty.parquet",
    ]:
    ...
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        raise SystemExit(1)
    print("AUDIT PASSED: Slice 7A SIDRA stitching, projection, high-dimensional bounding, and fitted ST-DFM contracts validated.")
