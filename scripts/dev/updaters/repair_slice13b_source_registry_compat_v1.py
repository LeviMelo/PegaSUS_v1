...
SOURCE_REGISTRY_APPLY_WRITE_FIXED = f'''write("src/pegasus/she/source_registry.py", r'''
{SOURCE_REGISTRY_FIXED.strip()}
''')'''
...
def patch_source_fields_yaml(path: Path) -> None:
    text = path.read_text(encoding="utf-8")
    if "registry_version:" not in text:
        text = text.replace(
    ...
    replacements = {
        'cause_chain_norm: {carrier: Deaths, unit: json': 'cause_chain_norm: {carrier: AuditMetadata, unit: json',
        'secondary_icd_norm_json: {carrier: HospitalAdmissions, unit: json': 'secondary_icd_norm_json: {carrier: AuditMetadata, unit: json',
        'classification_tuple: {carrier: ContextCells, unit: json': 'classification_tuple: {carrier: AuditMetadata, unit: json',
    }
...
def patch_substrate(path: Path) -> None:
    text = path.read_text(encoding="utf-8")
    start = "# ---- Slice 13B source-registry compatibility bridge ----"
    end = "# ---- End Slice 13B source-registry compatibility bridge ----"
    if start in text and end in text:
        text = re.sub(re.escape(start) + r".*?" + re.escape(end), SUBSTRATE_COMPAT_BLOCK.strip(), text, flags=re.S)
    else:
        text = text.rstrip() + "\n\n" + SUBSTRATE_COMPAT_BLOCK.strip() + "\n"
    path.write_text(text.rstrip() + "\n", encoding="utf-8", newline="\n")


def patch_apply_script(path: Path) -> None:
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    if "registry_version: \"1.0\"" not in text:
        text = text.replace(
            'registry_id: "source_field_registry_v1"\ndefault_unknown:',
            'registry_id: "source_field_registry_v1"\n'
            'registry_version: "1.0"\n'
    ...
    for old, new in {
        ...
        text = text.replace(old, new)
    pattern = r'write\("src/pegasus/she/source_registry\.py", r\'\'\'.*?\n\'\'\'\)\n\nwrite\("tests/unit/test_registry_source_fields_yaml\.py"'
    replacement = SOURCE_REGISTRY_APPLY_WRITE_FIXED + '\n\nwrite("tests/unit/test_registry_source_fields_yaml.py"'
...
def main() -> None:
    ...
    if not substrate.exists():
        raise FileNotFoundError(substrate)
    if not source_fields.exists():
        raise FileNotFoundError(source_fields)

    write_text("src/pegasus/she/source_registry.py", SOURCE_REGISTRY_FIXED)
    patch_substrate(substrate)
