...
def write_hsic_bundle() -> None:
    write(
        "src/pegasus/output/hsic_bundle.py",
        r'''
...
def write_hsic_fixture() -> None:
    observations = []
    years = list(range(2011, 2023))
    municipalities = [f"270{i:03d}" for i in range(1, 11)]
    for y_idx, year in enumerate(years):
        for m_idx, mun in enumerate(municipalities):
            covariate = round(0.10 + 0.015 * y_idx + 0.011 * m_idx, 6)
            centered = covariate - 0.25
            # Near-monotone residual signal with a small deterministic ripple; keeps fixture HSIC positive.
            residual = round(1.75 * centered + 0.01 * ((y_idx + m_idx) % 3 - 1), 6)
            observations.append({
                "municipality_cod6": mun,
                "year": year,
                "residual": residual,
                "covariate": covariate,
            })
    write_json(
        "tests/fixtures/pirs/pirs_slice9a_hsic_fixture.json",
        {
            "panel_type": "annual_municipal_panel",
            "spatial_blocks": len(municipalities),
            "temporal_blocks": len(years),
            "seed": 20260611,
...
def main() -> None:
    ...
    write_hsic_fixture()
    trim_cli()
    print("Slice 9A repair v1 applied: HSIC fixture support, standalone manifest contract, and telemetry statuses fixed.")
