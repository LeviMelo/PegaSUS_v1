...
def orient_edge(record: LinkRecord, data: dict[str, np.ndarray]) -> LinkRecord:
    ...
    if record.edge_type == "lagged_directed":
        return replace(record, warnings=record.warnings + ("directed_by_time_precedence",))
    ...
    if result is None:
        return replace(record, warnings=record.warnings + ("orientation_undirected_unidentifiable",))
...
def orient_links(records: list[LinkRecord], data: dict[str, np.ndarray]) -> list[LinkRecord]:
    """Apply Rung-1 orientation to every edge (MSD-III §IV)."""
    return [orient_edge(r, data) for r in records]
