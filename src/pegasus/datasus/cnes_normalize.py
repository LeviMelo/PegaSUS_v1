...
def normalize_cnes_st_events(
    *, input_path: str | Path, output_path: str | Path, source_manifest_hash: str
    ...
    """Batch CNES-ST raw→canonical SHE normalizer (MSD §2.4.4, §2.4.0.4).

    SHE-NORM-01 / XCUT-02: single record-level decode authority,
    ``normalize_cnes_st_record``. It routes each field through the shared
    decoders (``clamp_bool`` for the boolean service-flag vector — values > 1 are
    InvalidFlagState, never coerced to true; ``filter_cnpj`` for the §2.4.0.5
    linkage gate) and emits the typed QTLEIT bed primitives plus the full
    capacity/flag vectors as JSON. No generic ``capacity_total_observed`` is
    produced (§2.4.4). This batch function maps that authority over the raw rows;
    it does not decode anything itself. (The prior parallel vectorized
    implementation produced byte-identical output and has been removed to keep a
    ...
    for record in rows:
        try:
            capacity_components.update(json.loads(record["capacity_vector_json"]).keys())
