...
def _structural_adjacency_cod6() -> dict[str, tuple[str, ...]] | None:
    """Structural queen-contiguity fallback (SPG default), keyed by municipality_cod6.

    Makes Moran's I (and the §3.12.3 n_eff correction) live-by-default from the
    single declared structural graph instead of silently absent whenever an intent
    omits an adjacency path.
    """
    try:
        adjacency = structural_cod6_adjacency()
    except Exception:
        return None
    return adjacency or None
